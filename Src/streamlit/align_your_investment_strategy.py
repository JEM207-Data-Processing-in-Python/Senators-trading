...
def equity_alignment_politician_sector(list_of_politicians, list_of_unique_sectors, data_general, data_user):
    """
    Compare the investment strategies of a user and multiple politicians based on sector allocation.
    It evaluates the alignment of investments between the user and each politician and visualizes
    the results with pie charts representing sector-based investment distributions.

    Parameters:
    - list_of_politicians (List[str]): A list of politician names to compare against the user's strategy.
    - list_of_unique_sectors (List[str]): A list of unique sector names for consistent color mapping in pie charts.
    - data_general (pd.DataFrame): The general investment data of politicians, sector-wise.
...
def equity_alignment_politician_instrument(list_of_politicians, list_of_unique_instruments, data_general, data_user):
    """
    Compare the investment strategies of a user and multiple politicians based on the type of financial instrument.
    It evaluates the alignment of investments between the user and each politician and visualizes the results
    with pie charts representing instrument-based investment distributions.

    Parameters:
    - list_of_politicians (List[str]): A list of politician names to compare against the user's strategy.
    - list_of_unique_instruments (List[str]): A list of unique financial instrument names for consistent color mapping in pie charts.
    - data_general (pd.DataFrame): The general investment data of politicians, categorized by financial instruments.
    - data_user (pd.DataFrame): The user's investment data categorized by financial instruments.
