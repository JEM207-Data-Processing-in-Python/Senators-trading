"""
This script contains the functions that create the helpful tables that can be
later displayed.
"""
import pandas as pd


def top_five_purchased_stocks(data: pd.DataFrame, politician: str,
                              quoteType: str) -> pd.DataFrame:
    """
    Create a table with the top 5 purchased stocks and the last transactions
    for a given politician and quoteType.

    :param data: pd.DataFrame - A pandas DataFrame containing trading data with
    columns such as "Politician", "quoteType", "Transaction", "Invested",
    "Ticker", etc.
    :param politician: str - The name of the politician for whom the top 5
    purchased stocks need to be calculated.
...
def top_five_sold_stocks(data: pd.DataFrame, politician: str,
                         quoteType: str) -> pd.DataFrame:
    """
    Create a table with the top 5 sold stocks and the last transactions for a
    given politician and quoteType.

    :param data: pd.DataFrame - A pandas DataFrame containing trading data with
    columns such as "Politician", "quoteType", "Transaction", "Invested",
    "Ticker", etc.
    :param politician: str - The name of the politician for whom the top 5 sold
    stocks need to be calculated.
    :param quoteType: str - The type of financial instrument (e.g., "EQUITY").
...
def data_for_strategy_align_type(data: pd.DataFrame) -> pd.DataFrame:
    """
    Creates a table that aligns the total investments by type (e.g., 'EQUITY',
    'BOND') for each politician.

    :param data: pd.DataFrame - A pandas DataFrame containing trading data with
    columns such as "Politician", "quoteType", and "Transaction".

    :return: pd.DataFrame - A pandas DataFrame containing alignment data for
    each politician and quoteType, including the percentage of total investment
    for each type.
    """
    total_purchase_df = (
    ...
    help_df_type = total_purchase_type_df.merge(total_purchase_df, how="left",
                                                on="Politician")
    ...
    final_table = cross_joined_df.merge(help_df_type, how="left",
                                        on=["Politician", "quoteType"]).fillna(0)
...
def data_for_strategy_align_sector(data: pd.DataFrame) -> pd.DataFrame:
    """
    Creates a table that aligns the total investments by sector for each
    politician in the 'EQUITY' quoteType.

    :param data: pd.DataFrame - A pandas DataFrame containing trading data with
    columns such as "Politician", "sectorKey", "quoteType", and "Transaction".

    :return: pd.DataFrame - A pandas DataFrame containing alignment data for
    each politician and sector, including the percentage of total investment
    for each sector.
    """
    total_purchase_type_df = (
        data[data["Transaction"] == "Purchase"]
    ...
    help_df_sector = total_purchase_sector_df.merge(total_purchase_equity_df,
                                                    how="left", on="Politician")
    ...
    final_table = cross_joined_df.merge(help_df_sector, how="left",
                                        on=["Politician", "sectorKey"]).fillna(0)
