...
def load_data(filepath: str, columns: List[str]) -> pd.DataFrame:
    """
    Helper function to load a dataset from a CSV file. If the file does not exist, create it with the given columns.
    ...
    Returns:
        pd.DataFrame: The loaded DataFrame or an empty DataFrame with the specified columns if the file doesn't exist.
    """
    try:
...
def get_last_current_data(current_data: pd.DataFrame) -> pd.DataFrame:
    ...
    Parameters:
    - current_data: A pandas DataFrame containing the current senators trading data.
...
def delete_exclude_tickers(exclude_tickers: pd.DataFrame, current_data: pd.DataFrame) -> pd.DataFrame:
    """
